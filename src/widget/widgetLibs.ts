/**
 * Browser libraries a widget can ask for with `libs:` in its frontmatter.
 *
 * A widget frame has no network and an opaque origin, so a library can reach it only one
 * way: inlined whole as a `<script>` in the frame's document. That makes every entry
 * here a per-widget cost (the source is parsed once per frame that requests it) and a
 ...
 * entry, and document the global and its scope in the `widgets` skill.
 */

import plotlySource from "plotly.js-gl3d-dist-min/plotly-gl3d.min.js?raw";

export interface WidgetLib {
	/** The id a widget writes under `libs:`. */
	id: string;
	/** Shown in notices. */
	displayName: string;
 ...
 */
export const WIDGET_LIBS: Readonly<Record<string, WidgetLib>> = {
	plotly: {
		id: "plotly",
		displayName: "Plotly",
		global: "Plotly",
...
export function resolveWidgetLibs(ids: readonly string[]): { sources: string[]; unknown: string[] } {
